...
const styles = StyleSheet.create({
  ...
  companyLogo: { width: 100, height: 30, marginBottom: 10 },
  companyDetails: { fontSize: 10, color: '#374151', lineHeight: 1.3 },
  reportInfo: {
    width: '35%',
    alignItems: 'flex-end',
  },
  ...
  infoLabel: { fontSize: 10, color: '#374151', width: 80, textAlign: 'right', marginRight: 5 },
  infoValue: { fontSize: 10, color: '#374151', fontWeight: 'bold', width: 100, textAlign: 'right' },
  customerSection: {
    marginBottom: 25,
  },
  customerName: {
  ...
  sectionTitle: { fontSize: 12, fontWeight: 'bold', color: '#374151', marginBottom: 6 },
  partsTableContainer: {
    marginBottom: 25,
  },
  tableHeader: {
  ...
  thankYou: { fontSize: 12, fontWeight: 'bold', color: '#374151', textAlign: 'center', marginTop: 30 },
})
...
const InstallationPDF: React.FC<InstallationPDFProps> = ({ service }) => {
  const reportNumber = service.quotation_number || `INS-${service.id.slice(-6).toUpperCase()}`
  ...
  return (
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        {/* Installation Details / Description */}
        <View style={{ marginBottom: 25 }}>
          <Text style={styles.sectionTitle}>Installation Details</Text>
          <Text style={styles.descriptionParagraph}>
            {service.description || 'No specific installation details provided.'}
          </Text>
        </View>

        {/* Parts Used Section */}
        {service.parts_used && service.parts_used.length > 0 && (
          <View style={styles.partsTableContainer}>
            <Text style={styles.sectionTitle}>Parts/Materials Used for Installation</Text>
            ...
            {service.parts_used.map((part, index) => (
              <View style={styles.tableRow} key={index}>
                <Text style={[styles.tableCell, styles.colDescription]}>{part.name}</Text>
                <Text style={[styles.tableCell, styles.colQty]}>{part.quantity}</Text>
                <Text style={[styles.tableCell, styles.colCost]}>{formatPeso(part.cost * part.quantity)}</Text>
              </View>
            ))}
          </View>
        )}
        ...
        {/* Thank you */}
        <Text style={styles.thankYou}>--- END OF INSTALLATION REPORT ---</Text>
      </Page>
    </Document>
  )
}

export default InstallationPDF 