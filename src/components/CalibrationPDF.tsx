...
interface CalibrationPDFProps {
  service: CompletedService
}

const CalibrationPDF: React.FC<CalibrationPDFProps> = ({ service }) => {
  const reportNumber = service.quotation_number || `CAL-${service.id.slice(-6).toUpperCase()}`
  ...
  return (
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        {service.notes && (
          <View style={{ marginBottom: 25 }}>
            <Text style={styles.sectionTitle}>Technician Notes / Calibration Results</Text>
            <Text style={styles.descriptionParagraph}>{service.notes}</Text>
          </View>
        )}

        {/* Cost Breakdown (Optional for Calibration, but consistent structure) */}
        ...
        {/* Thank you */}
        <Text style={styles.thankYou}>--- END OF CALIBRATION REPORT ---</Text>
      </Page>
    </Document>
  )
}

export default CalibrationPDF 